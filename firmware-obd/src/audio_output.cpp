...
esp_err_t set_sample_rate(uint32_t sample_rate)
{
    ...
    if(err != ESP_OK || s_sample_rate == sample_rate) return err;
    ...
    if(err == ESP_OK) err = i2s_channel_enable(s_tx_channel);
    if(err == ESP_OK) s_sample_rate = sample_rate;
    return err;
}
...
{
    if(s_effect_active.exchange(true)) return ESP_ERR_INVALID_STATE;
    const esp_err_t err = set_sample_rate(sample_rate);
    if(err != ESP_OK) s_effect_active.store(false);
    return err;
}
