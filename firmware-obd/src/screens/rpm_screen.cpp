...
static void update_alert_state(int rpm, bool valid)
{
    const uint16_t threshold = settings::get().shiftRpm;
    const bool active = valid && (s_alert_active
        ? rpm >= static_cast<int>(threshold - kShiftRpmHysteresis)
        : rpm >= threshold);
...
static void update_visuals(int rpm, bool valid)
{
    if(!valid) rpm = 0;
    if(rpm < 0) rpm = 0;
    ...
    char value[12];
    if(valid) snprintf(value, sizeof(value), "%d", rpm);
...
void showRpm()
{
    ...
    // Moldura preta espessa e visor interno ̢mbar, reproduzindo o painel automotivo.
